...
(function (window, exports) {
  ...
  function unserialize (data) {
    ...
    // *      '🏢'
    var that = this, datalength,
      error = function (type, msg, filename, line) {
        throw new window[type](msg, filename, line);
      },
      read_until = function (data, offset, stopchr) {
        var i = 2, buf = [], chr = read_chrs(data, offset, 1)[1];
        ...
        while (chr != stopchr) {
          ...
          buf.push(chr);
          chr = read_chrs(data, offset + (i - 1), 1)[1];
          i += 1;
        }
      ...
      read_chrs = function (data, offset, length) {
        ...
        let arrData = utf8Encode.encode(data);
        let buf = arrData.slice(offset-1,offset+length-1);
        return [buf.length, new TextDecoder().decode(buf)];
      },
      ...
      _unserialize = function (data, offset) {
        ...
        dtype = (read_chrs(data, offset, 1)[1]).toLowerCase();
  ...
  function getClass(data, offset) {
    ...
    body = read_chrs(data, offset - 1, parseInt(res[1]))[1];
  ...
  function getString(data, offset) {
    ...
    readData = read_chrs(data, offset, parseInt(stringlength, 10));
    chrs = readData[0];
    readdata = readData[1];
    offset += chrs + 2;
    ...
    datalength = (new TextEncoder).encode(data).length;
